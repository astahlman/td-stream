...
(defn create-signals [tweets] ((:update (all-signals nil)) tweets))
(defn update-signals [signals tweets] ((:update signals) tweets))
...
(defn- partition-tweets-by-team [tweets]
  (into {}
        (for [{:keys [simple-name abbrev]} teams]
          [abbrev
           (seq (filter #(re-find (re-pattern (str "(?i)" simple-name)) (:text %)) tweets))])))
     ...
     {:magnitude (fn [team]
      ...
      :update (fn [tweets]
                (let [num-tweets (+ (count tweets) (or num-tweets 0))
                      ...
                      mentions-by-team (into {}
                                             (for [[team mentions] (partition-tweets-by-team tweets)]
                                               [team (count mentions)]))]
                  (bucket
                   num-tweets
                   total-chars
...
(defn- all-signals [buckets-by-time]
  (let [bucket-sz-ms 5000
        window-sz-ms 100000
        ;; We intentionally drop the most recent bucket, as it's value
                  ...
                  (trim-to-window
                   (reduce (fn [buckets [t tweetz]]
                             (if-let [bucket (get buckets t)]
                               ...
                               (assoc buckets t (create-bucket tweetz))))
                           buckets-by-time
                           tweets-by-time)))))}))
